...
local function initialize(eventCode, addOnName)
    ...
    settings = ZO_SavedVars:New("FreeSlotsSettings" , settingsVersion, nil, settingsDefaults, nil)

    FreeSlotsUI:SetAnchor(TOPLEFT, GuiRoot, TOPLEFT, settings.wm.x, settings.wm.y)
    FreeSlotsUI:SetWidth(settings.wm.width)
    FreeSlotsUI:SetHeight(settings.wm.height)
...
function FreeSlots.OnMoveStop(self)
    settings.wm.x = self:GetLeft()
    settings.wm.y = self:GetTop()
    settings.wm.width = self:GetWidth()
    settings.wm.height = self:GetHeight()
end
